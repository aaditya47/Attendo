...
public class TodoController {
	...
	public ResponseEntity<TodoDTO> createTodo(@RequestBody TodoDTORequests todo){
		...
		try {
			...
			TodoDTO result=todoRepo.save(new TodoDTO(res_stud, todo.GetSubject(), todo.GetDdate(),res_teach,todo.GetAttendedhours(),todo.GetNoofhours()));
	...
	@PostMapping(value="/PastLeaves",consumes = MediaType.APPLICATION_JSON_VALUE, 
	...
	public ResponseEntity<?> createPL(@RequestBody PastLeavesRequests todo){
		try {
			...
			if(todos!=null) {
				...
				PastLeaves result=PLRepo.save(new PastLeaves(todoOptional_stud, todoOptional_teach, todo.GetDOS(),todo.GetDOE(),todo.GetReason(),todo.GetApproval(),todo.GetCertLink()));
				return new ResponseEntity<>(result,HttpStatus.OK);
			}
		}
			catch(Exception e) {
	...
	//Student Login
		...
		//Teacher Login
				...
				@PutMapping(value="/PastLeaves/Approve",consumes = MediaType.APPLICATION_JSON_VALUE, 
						  produces = MediaType.APPLICATION_JSON_VALUE)
					...
					try {
						...
						if(todos==null) {
							...
							String content = "<b>Dear"+" "+res.GetName()+"</b>,<br><i>Below is your Leave Status</i>"
							        + "<br><img src='cid:image001' width='300px' height='300px'/><br><b>"+todos.GetApproval()+"</b>";
							helper.setText(content, true);
							 
							FileSystemResource resource = new FileSystemResource(new File("E:\\Attendance\\src\\main\\java\\in\\ncpproject\\Attendance\\controller\\picture.png"));
						...
						if(val.charAt(0)=='T') {
							...
							if(todo.getEmailId().equals(res.GetEmail())) {
								...
								String content = "<b>Dear"+" "+res.GetName()+"</b>,<br><i>Your Password</i>"
								        + "<br><img src='cid:image002' width='300px' height='300px'/><br><b>"+resL.GetPassword()+"</b>";
								helper.setText(content, true);
